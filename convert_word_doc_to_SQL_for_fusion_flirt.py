...
def clean(file):
    ...
    # Empty string to store the SQL command for questions table
    SQL_Command = ""
    # Empty string to store the SQL command for answers table
    SQL_Command_answers = ""
    ...
    for title in clean_data:      
        ...
        for question in clean_data[title]:
            ...
            # Format of SQL table
            "question_ID","questionNumber","questionText", "fk_survey_ID"
            # Seperate each part :)
    ...
    f.write(SQL_Command)
    f.close()
    f= open("SQL code survey_tb.txt","w", encoding="utf-8")
    f.write(SQL_Command_Groups)
    f.close()
    f= open("SQL code answers_tb.txt","w", encoding="utf-8")
    f.write(SQL_Command_answers)
    f.close()


    return SQL_Command, SQL_Command_Groups, SQL_Command_answers

# Displays the contents of the file
SQL_Command,SQL_Command_Groups,SQL_Command_answers = clean(f)

# Writes code for creating the entire database
f = open("SQL Code For Creating Tables 3rd Normal Form.txt", "r")
createDatabaseCode = f.readlines()
f.close()
createDatabaseCode = "".join(createDatabaseCode)
#SQL_Command_Groups
#SQL_Command
#SQL_Command_answers
fullCode = """"""
fullCode += createDatabaseCode
fullCode += SQL_Command_Groups
fullCode += SQL_Command
fullCode += SQL_Command_answers
print(fullCode)
